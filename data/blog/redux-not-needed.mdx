...
This looks a lot nicer than useEffect. You do have to learn the react-query library though. It's worth mentioning a lot of libraries are changing to `useSignal` like Solid.js, preact, and qwik. You can use preact signals in react apps by using `@preact/signals-react` library.
...
If using `react-query`, and `useState` is not enough for your application, then you can use `Zustand`, and `Jotai`.

something about atoms
`Zustand` is a library that is used to handle state in the frontend. It is a lot simpler than using `redux`. You can use `Zustand` like:
